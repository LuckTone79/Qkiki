...
## v1.36.0-20260706 - 프로젝트 대화 개별 삭제

- 프로젝트 대화 카드에 모바일에서도 바로 보이는 `삭제` 버튼을 추가했습니다.
- `프로젝트에서만 제거`는 원본 대화와 결과를 보존하고 프로젝트 연결과 수집 항목만 제거합니다.
- `원본까지 영구 삭제`는 위험 안내와 2차 확인을 거친 후에만 실행됩니다.
- 새 API는 로그인 사용자의 프로젝트·대화 소유권을 검증하고 트랜잭션으로 연결을 해제합니다.
