...
function getOAuthErrorMessage(errorCode: string | null, language: AppLanguage) {
  ...
  if (errorCode === "oauth_failed") {
    return localize(language, {
      ...
      ko: "소셜 로그인에 실패했습니다. 다시 시도해주세요.",
      ja: "ソーシャルログインに失敗しました。もう一度お試しください。",
      es: "El inicio de sesión social falló. Inténtalo de nuevo.",
    });
  }
  ...
  if (errorCode === "google_secure_browser_required") {
    return localize(language, {
      en: "Social sign-in is blocked inside some in-app browsers. Open this page in Chrome, Safari, or another system browser and try again.",
      ko: "일부 인앱 브라우저에서는 소셜 로그인이 차단될 수 있습니다. 크롬 또는 사파리 같은 기본 브라우저로 열어서 다시 시도해주세요.",
      ja: "一部のアプリ内ブラウザーではソーシャルログインがブロックされることがあります。ChromeやSafariなどの標準ブラウザーで開いて再度お試しください。",
      es: "El inicio de sesión social puede bloquearse en algunos navegadores integrados. Abre esta página en Chrome, Safari u otro navegador del sistema y vuelve a intentarlo.",
    });
  }
...
function getAuthReasonMessage(reason: string | null, language: AppLanguage) {
  ...
  if (reason === "trial_login_required") {
    return localize(language, {
      ...
      es: "Este dispositivo no puede iniciar otra prueba. Inicia sesión para continuar.",
    });
  }
...
const oauthText = {
  ...
  ja: {
    ...
    checkEmailDescription:
      "確認リンクを送信しました。リンクをクリックしてアカウント作成を完了してください。",
  },
  es: {
    ...
    checkEmailDescription:
      "Hemos enviado un enlace de confirmación. Haz clic en él para terminar de crear tu cuenta.",
  },
} as const;
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <a
          ...
          className="block w-full rounded-md bg-[#FEE500] px-4 py-2.5 text-center text-sm font-semibold text-[#191919] hover:brightness-95"
        >
          {oauthCopy.kakao}
        </a>
      </div>
